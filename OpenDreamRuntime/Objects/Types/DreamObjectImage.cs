...
public sealed class DreamObjectImage : DreamObject {
    ...
    public DreamObjectImage(DreamObjectDefinition objectDefinition) : base(objectDefinition) {
        if (objectDefinition.IsSubtypeOf(ObjectTree.MutableAppearance)) {
            ...
            Entity = EntityManager.SpawnEntity(new MapCoordinates(0, 0, MapId.Nullspace)); //spawning an entity in nullspace means it never actually gets sent to any clients until it's placed on the map, or it gets a PVS override
