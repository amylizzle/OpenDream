...
internal sealed class DreamPlane(IRenderTexture mainRenderTarget) : IDisposable {
    ...
    public readonly SortedSet<FastRenderData> Sprites = new();
    ...
    public void Draw(DreamViewOverlay overlay, DrawingHandleWorld handle, Box2 worldAABB) {
        ...
        if (_temporaryRenderTarget != null) {
            ...
            handle.RenderInRenderTarget(_temporaryRenderTarget, () => {
                if (Master is null || !Enabled)
                    return;

                Master.TextureOverride = mainRenderTarget.Texture;
                var fastMaster = Master.asFastRenderData(overlay, handle, Vector2.Zero);
                // var texture = Master.GetTexture(overlay, handle);
                // Master.TextureOverride = null;
                if (fastMaster.Texture == null)
                    return;

                handle.UseShader(overlay.GetBlendAndColorShader(fastMaster, useOverlayMode: true));
                ...
                handle.DrawTextureRect(fastMaster.Texture, new Box2(Vector2.Zero, fastMaster.Texture.Size), Master?.ColorToApply);
            }, new Color());
        }
    }
